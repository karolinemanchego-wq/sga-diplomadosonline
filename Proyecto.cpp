...
public:
    ...
    bool evaluarAprobacion(const std::vector<double>& notas) override {
        // El promedio académico  se divide entre 3 evaluaciones fijas
        double suma = 0;
...
public:
    ...
    void registrarAlumno(Alumno* alumno) {
        bool yaExiste = false;
        for (Alumno* a : listaAlumnos) {
            switch (a->getCedula() == alumno->getCedula()) {
                case true: yaExiste = true; break;
            }
        }

        switch (yaExiste) {
            case true:
                std::cout << "Error: La cedula " << alumno->getCedula() << " ya se encuentra registrada.\n";
                delete alumno; 
                return;
            case false:
    ...
    while (enEjecucion) {
        ...
        switch (optChar) {
            ...
            case '3': {
                ...
                std::cout << "Ingrese las notas separadas por comas (ej: 10,15,20): ";
